...
TermState.prototype.eraseInDisplay = function(n) {
	var c = this.cursor, i, existingLine, newLine, self = this;
	var chLine = function() {
		newLine = self._createLine();
		self._applyAttributes(newLine, 0, self.columns);
		self.setLine(i, newLine);
	};
	switch(n || 0) {
		case "below":
		case "after":
		...
		case 2:
			for(i = 0; i < this.rows; i++) {
				existingLine = this.getLine(i);
				Array.prototype.push.apply(this._scrollback.str, [existingLine.str]);
				Array.prototype.push.apply(this._scrollback.attr, [existingLine.attr]);
				chLine();
			}
			return this;
	}
